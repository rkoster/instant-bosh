...
The system_domain and router_static_ips are substituted with resolved values.
...
Examples:
  ...
  ibosh cf manifest > manifest.yml           # Save to file`,
						Flags: []cli.Flag{
							&cli.StringFlag{
								Name:  "router-ip",
								Usage: "Static IP for the router (if not specified, requires BOSH env for auto-selection)",
							},
							&cli.StringFlag{
								Name:  "system-domain",
								Usage: "System domain for CF (defaults to <router-ip>.sslip.io)",
