				...
				Subcommands: []*cli.Command{
					{
						...
						Flags: []cli.Flag{
							&cli.StringFlag{
								Name:  "router-ip",
								Usage: "Static IP for router (auto-selected from cloud-config if not specified)",
							},
							&cli.StringFlag{
								Name:  "system-domain",
...
The system_domain and router_static_ip are substituted with resolved values.

Examples:
  ibosh cf manifest                           # Auto-select router IP (requires BOSH env)
  ibosh cf manifest --router-ip 10.245.0.34  # Use specific router IP
  ibosh cf manifest > manifest.yml           # Save to file`,
						Flags: []cli.Flag{
							&cli.StringFlag{
								Name:  "router-ip",
								Usage: "Static IP for router (if not specified, requires BOSH env for auto-selection)",
							},
							&cli.StringFlag{
								Name:  "system-domain",
								Usage: "System domain for CF (defaults to <router-ip>.sslip.io)",
