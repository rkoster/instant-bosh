				...
				Subcommands: []*cli.Command{
					{
						...
						Flags: []cli.Flag{
							&cli.StringFlag{
								Name:  "haproxy-ip",
								Usage: "Static IP for haproxy (auto-selected from cloud-config if not specified)",
							},
							&cli.StringFlag{
								Name:  "system-domain",
								Usage: "System domain for CF (defaults to <haproxy-ip>.sslip.io)",
							},
							&cli.BoolFlag{
								Name:  "dry-run",
					...
					{
						Name:  "manifest",
						Usage: "Output the interpolated CF deployment manifest",
						Description: `Output the CF deployment manifest with all ops files applied.

This is useful for external tools that want to inspect or build upon
the ibosh-generated CF manifest.

Variable credentials remain as placeholders (e.g. ((cf_admin_password)))
since they are generated by config-server during deployment.
The system_domain and haproxy_private_ip are substituted with resolved values.

Examples:
  ibosh cf manifest                            # Auto-select haproxy IP (requires BOSH env)
  ibosh cf manifest --haproxy-ip 10.245.0.34   # Use specific haproxy IP
  ibosh cf manifest > manifest.yml             # Save to file`,
						Flags: []cli.Flag{
							&cli.StringFlag{
								Name:  "haproxy-ip",
								Usage: "Static IP for haproxy (if not specified, requires BOSH env for auto-selection)",
							},
							&cli.StringFlag{
								Name:  "system-domain",
								Usage: "System domain for CF (defaults to <haproxy-ip>.sslip.io)",
						...
						Action: func(c *cli.Context) error {
							ui, _ := initUIAndLogger(c)
							opts := commands.CFManifestOptions{
								HaproxyIP:    c.String("haproxy-ip"),
								SystemDomain: c.String("system-domain"),
							}
